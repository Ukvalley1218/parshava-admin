...
export default function Subcategories() {
  ...
  const fetchSubcategories = async (page = currentPage, limit = pagination.limit) => {
    setLoading(true)
    setError(null)
    try {
  ...
  const handleUpdate = async (data) => {
    setFormLoading(true)
    try {
      const response = await updateSubcategory(selectedSubcategory._id, data)
      if (response.success !== false) {
  ...
  // Only show full error state if we have no subcategories at all
  if (error && subcategories.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-64">
        <AlertCircle className="w-12 h-12 text-red-400 mb-4" />
        <p className="text-gray-600 mb-4">{error}</p>
        <button onClick={() => fetchSubcategories(1)} className="btn-primary">Retry</button>
      </div>
    )
  }
    ...
    <div className="space-y-6 animate-fadeIn">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-1">Manage subcategories linked to categories</p>
        </div>
        <button
          onClick={() => { setSelectedSubcategory(null); setShowModal(true) }}
      ...
      {subcategories.length === 0 ? (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-12 text-center">
          ...
          <p className="text-sm text-gray-400">Create your first subcategory to get started</p>
        </div>
      ) : (
        <>
                ...
                <tbody className="divide-y divide-gray-100">
                  {subcategories.map((subcategory) => (
                    <tr key={subcategory._id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <span className="text-gray-600">{subcategory.category?.name || '-'}</span>
                      </td>
                      <td className="px-6 py-4">
                        <span className={`px-2 py-1 text-xs rounded-full ${subcategory.active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'}`}>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center justify-end gap-2">
                          <button
                            onClick={() => { setSelectedSubcategory(subcategory); setShowModal(true) }}
      ...
      <Modal
        isOpen={showModal}
        onClose={() => { setShowModal(false); setSelectedSubcategory(null) }}
        title={selectedSubcategory ? 'Edit Subcategory' : 'Add Subcategory'}
      >
        <SubcategoryForm
          subcategory={selectedSubcategory}
          categories={categories}
          onSubmit={selectedSubcategory ? handleUpdate : handleCreate}
          onCancel={() => { setShowModal(false); setSelectedSubcategory(null) }}
          loading={formLoading}
        />
      </Modal>
    </div>
